...
max_period_aimed = (
    10 * 365.25 * 86400
)  # In first_legs_search this is the orbital period to aim for
...
while True:
    ...
    if initial_point_search is True:
        ...
        print(
            "For now this part of the script has no end and will parse all V0 then offset lambda 0 by 10° modulo 2 * pi then iterate again forever, it should be stopped by the user when 'init_pos.csv' is considered big enough "
        )
        while True:
            ...
            for i in range(len(V_space)):
                ...
                if res.fun < tol_pos:
                    ...
                    rf = [-200 * AU, res.x[0], 0]
                    vf = [V_space[i], 0, 0]
                    rfper, vfper, info = state_at_perigee_from_rv(rf, vf, MU_ALTAIRA)
                    ...
                    invalid, shield_state_not_used = shield_state(
                        False, rf, vf, rf_min, vf_min, tof_init
                    )
    ...
    if sequence_search == True:
        ...
        if first_legs_search == True:
            ...
            shield_burned, V0_list, VFMIN_list, L_list, tp_list, pos_list = (
                read_init_pos("init_pos.csv")
            )
            ...
            while True:
                ...
                invalid, shield_burned = shield_state(
                    shield_burned, r_init, v_init, r1_fb1, v1_fb1, tof_fb1
                )
                ...
                if not invalid:
                    ...
                    if t + tof2 < t_max:
                        ...
                        invalid, shield_burned = shield_state(
                            shield_burned, r1_fb1, v1, r2, v2, tof2
                        )
                        ...
                        if not invalid:
                            ...
                            # saving flybys
                            save_flybys_to_csv(
                                flybys, filename="flyby_first_two_legs.csv"
                            )
                        ...
                        print(
                            "First two segments are already > t_max : ",
                            t / 86400 / 365.25,
                        )
                        break
        ...
        for depth_nb in range(sequence_search_depth):
            ...
            for n_fb in range(len(flybys) - 1):
                ...
                if n_fb > int(len(results_json) - 1):
                    ...
                    if conic == 0:
                        raise Exception(
                            "An arc is not conic, but the solve_solar_sail_arcs function is still TBD"
                        )
            ...
            if conic:
                ...
                if Draw_mission:
                    ...
                    sc_handle = ax.scatter(
                        r[0], r[1], r[2], color="r", s=40, label="SC @ t"
                    )
                ...
                for j in range(len(sol.t)):
                    ...
                    if alpha > np.pi / 2 + 0.0001 or alpha < -np.pi / 2 - 0.0001:
                        print("Alpha is outside control bounds :", alpha, "rad")
                    if beta > np.pi / 2 + 0.0001 or beta < -np.pi / 2 - 0.0001:
                        print("Beta is outside control bounds :", alpha, "rad")
    ...
    if (
        ...
        and not first_legs_search
        and not sequence_search
        and not solve_solar_sail_arcs
    ):
        break
if Draw_mission:
    plt.show(block=True)
